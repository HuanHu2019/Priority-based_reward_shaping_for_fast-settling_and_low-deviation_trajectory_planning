...
if len(s):
    ...
    sec_term_factor = 10 ** ( int(match.group(1)))
...
class Fuckingfly:
    ...
    def step(self, action):
        ...
        if done == True:
            ...
            self.Reward_for_settling_time = jisuan_settling_time_reward(self.initial,    self.end ,  self.record_flying_height)
            
        else:
                        
            shineng_now,xishu,xishu2,reward_pan,reward_pan_sec,reward_third,reward_forth = shinengjisuan(self.initial,    self.end ,   fuck_fu[-1], self.jietidijibu)
        ...
        return state_next_yuan.astype(np.float32) , reward_together , done , {}
...
def shinengjisuan(initial,end,feigao,jietidijibu):
        ...
        if juli > fenmu*0.5*tolerance:
            ...
            zhenjuli = juli - fenmu*0.5*tolerance
            
            cost_n_weinorm =  1 - np.clip(zhenjuli/fenmu,0,1) 
        
        cost_n = cost_n_weinorm
